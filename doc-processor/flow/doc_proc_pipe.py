...
from PDFGenerator import PDFGenerator  # ✅ 추가된 import
...
# [PDF 생성 태스크 추가]
# =========================================
@task(name="generate_pdf_from_images")
def generate_pdf_from_images(image_descriptions: Dict[str, Any],
                             output_pdf: str = "./extracted_pdf/pages_added_full.pdf") -> Dict[str, Any]:
    """
    이미지별로 생성된 설명(또는 텍스트)을 페이지 단위로 PDF로 저장합니다.
    - PDFGenerator.PDFGenerator.add_page(text)를 각 페이지마다 호출
    - 마지막에 pdf_gen.generate(output_pdf)
    """
    logger = get_run_logger()
    try:
        # 페이지 번호 기준 정렬
        items = []
        for img_path, info in image_descriptions.get("image_descriptions", {}).items():
            pg = info.get("page_number", 0)
            desc = info.get("description", "") or ""
            items.append((pg, desc))
        items.sort(key=lambda x: x[0])

        pdf_gen = PDFGenerator(font_size=8)
        page_count = 0
        for pg, desc in items:
            text = f"[페이지 {pg}]\n" + (desc.strip() if desc else "(설명 없음)")
            pdf_gen.add_page(text)
            page_count += 1

        # PDF 생성
        pdf_gen.generate(output_pdf)
        logger.info(f"📘 이미지 설명 기반 PDF 생성 완료: {output_pdf} (총 {page_count}페이지)")
        return {"output_pdf": output_pdf, "pages": page_count}
    except Exception as e:
        logger.error(f"❌ PDF 생성 실패: {str(e)}")
        return {"error": str(e), "output_pdf": output_pdf, "pages": 0}
...
def document_processing_pipeline(document_path: str, skip_image_processing: bool = False, max_pages: int = None, document_type: str = 'common'):
    ...
    try:
        ...
        # 4단계: Milvus 비활성화 → PDF 생성으로 대체
        logger.info("🗄️ 4단계 대체: 이미지 설명을 PDF로 생성")
        pdf_result = generate_pdf_from_images(description_result)
        ...
        saved_chunks = 0
        # ygkim if db_initialized and doc_metadata and vector_result.get("total_documents", 0) > 0:
        if db_initialized and doc_metadata:
            ...
            try:
                ...
                update_document_processing_status(
                    ...
                    # ygkim vector_count=vector_result['total_documents']
                )
                
                # 작업 완료 처리
                if job_id:
                    # ygkim complete_processing_job(job_id, saved_chunks, vector_result['total_documents'])
                    complete_processing_job(job_id, saved_chunks, 1 )
                ...
                if doc_metadata and job_id:
                    try:
                        ...
                        # ygkim complete_processing_job(job_id, saved_chunks, vector_result['total_documents'], str(e))
                        complete_processing_job(job_id, saved_chunks, 1, str(e))
                    except:
                        pass
        ...
        # ygkim logger.info(f"   - Vector DB 항목 수: {vector_result['total_documents']}")
        logger.info(f"   - PostgreSQL 저장 청크 수: {saved_chunks}")
        # ygkim logger.info(f"   - 사용된 임베딩 모델: {vector_result['embedding_model']}")
        # ygkim logger.info(f"   - 임베딩 API 버전: {vector_result['embedding_api_version']}")
