    ...
    try:
        with next(get_db_session()) as session:
            ...
            result = job_service.create_job(
                ...
                total_steps=4  # 텍스트 추출, 이미지 캡처, GPT 설명, PDF 생성
            )
    ...
    try:
        with next(get_db_session()) as session:
            ...
            if success:
                progress_percent = 0
                if completed_steps is not None:
                    # total_steps는 4로 고정 (텍스트 추출, 이미지 캡처, GPT 설명, PDF 생성)
    ...
    try:
        ...
        for image_path in image_paths:
            try:
                ...
                '''
                이 이미지의 내용을 자세히 설명해주세요. 텍스트, 차트, 그래프, 표, 사진, 그림 등 모든 요소를 포함하여 설명해주세요. 
...
# ===============================
# 4단계: PDF 생성 (PDFGenerator.py 사용)
# ===============================
@task(name="generate_pdf_with_images")
def generate_pdf_with_images(
    ...
    """추출된 텍스트와 이미지를 PDF로 생성합니다."""
    logger = get_run_logger()
    logger.info(f"📄 PDF 생성 시작 (PDFGenerator.py 사용)")
    
    try:
        # PDF Generator 초기화
        pdf_gen = PDFGenerator()
        
        # 페이지별로 PDF에 이미지와 설명 추가
        for image_path in image_paths:
            try:
                # 이미지 파일 로드
                ...
                if page_key in extracted_text.get("extracted_text", {}):
                    text_content = extracted_text["extracted_text"][page_key].get("text", "")
                
                # 이미지 설명 찾기
                image_description = ""
                if image_path in image_descriptions.get("image_descriptions", {}):
                    image_description = image_descriptions["image_descriptions"][image_path].get("description", "")
                ...
                logger.info(f"✅ 페이지 {page_number} PDF에 추가 완료")
                
            except Exception as e:
                logger.error(f"❌ 페이지 {page_number} PDF 추가 실패: {str(e)}")
                continue
        
        # PDF 파일 저장
        output_pdf_path = "./extracted_pdf/pages_added_full.pdf"
        os.makedirs(os.path.dirname(output_pdf_path), exist_ok=True)
        pdf_gen.generate(output_pdf_path)
...
@flow(
    ...
    description="4단계 문서 처리 파이프라인: 텍스트 추출, 이미지 캡처, GPT 설명, PDF 생성"
)
def document_processing_pipeline(
    ...
    # 1. 문서 메타데이터 생성
    metadata = create_document_metadata(document_path, document_type)
    doc_id = metadata["doc_id"]
    
    # 2. 처리 작업 로그 생성
    job_id = create_processing_job(doc_id, "prefect_flow_run")
    
    try:
        # 3. 문서 처리 상태 업데이트 (처리 중)
        update_document_processing_status(doc_id, "processing")
        update_job_progress(job_id, "텍스트 추출 중", 1)
        
        # 4. 텍스트 추출
        text_result = extract_text_from_document(document_path, max_pages)
        update_job_progress(job_id, "이미지 캡처 중", 2)
        
        # 5. 이미지 캡처
        image_result = capture_page_images(document_path, max_pages=max_pages)
        update_job_progress(job_id, "GPT 설명 생성 중", 3)
        
        # 6. 이미지 설명 생성
        description_result = generate_image_descriptions(image_result["image_paths"])
        update_job_progress(job_id, "PDF 생성 중", 4)
        
        # 7. PDF 생성 (Milvus 대신 PDFGenerator 사용)
        pdf_result = generate_pdf_with_images(
            text_result, 
            description_result, 
        ...
        update_document_processing_status(
            doc_id, 
            "completed", 
            processed_pages=len(text_result["extracted_text"]),
            output_pdf_path=pdf_result["pdf_path"]
        )
        
        # 9. 처리 작업 완료
        complete_processing_job(
            job_id, 
            success_count=len(text_result["extracted_text"]),
            total_count=len(text_result["extracted_text"])
        )
        ...
        # 실패 상태 업데이트
        update_document_processing_status(doc_id, "failed", error_message=str(e))
...
if __name__ == "__main__":
    # 테스트 문서 경로
    sample_document = "sample_documents/test.pdf"  # 적절한 경로로 변경 필요
    
    # 파이프라인 실행 (개발 모드)
    result = document_processing_pipeline(
        document_path=sample_document,
        max_pages=3,  # 테스트를 위해 페이지 수 제한
        document_type="test"
    )
